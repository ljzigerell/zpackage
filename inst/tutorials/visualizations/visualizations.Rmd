...
install.packages("learnr", dependencies=TRUE, repos="http://cran.us.r-project.org")
library(learnr)
...
Let's get data for two variables: X and Y. Let's use the combine function (c) to combine numbers, and then let's use the assign function (<-) to assign those numbers to X and Y. Then let's plot a scatterplot of X and Y:
...
X <- c(1,2,3,4,5)
Y <- c(4,5,8,9,7)
plot(X, Y, pch=21, cex=4, bg="lightblue", col="blue4", lwd=2.5)

```

---

This is an example of plotting with the base functions of R, which are available when a session of R is opened. But packages of functions can be added to base R...
...
First, R packages must be put on your computer using a command such as **install.packages("tidyverse")**, which will download the **tidyverse** package to your computer. The command below will install dependent packages and draw from the CRAN repository:

```{r tidyverse_install, echo=TRUE, eval=TRUE, exercise=FALSE, message=TRUE, warning=TRUE}
...
```{r tidyverse_library, echo=TRUE, exercise=FALSE, message=TRUE, warning=TRUE}
...
Let's try to read a CSV file by going and getting the file:

```{r readCSV, exercise=TRUE, message=TRUE, warning=TRUE}
...
ggplot(data=GUNIMP, aes(x=PID, y=PE)) +
  geom_col(linewidth=1.1, width=0.8, color="black", 
...
ggplot(data=GUNIMP, aes(x=PID, y=PE)) +
  geom_errorbar(aes(ymin=CILO, ymax=CIHI), linewidth=0.75, width=0.25, color="orange") +
  geom_point(size=5, shape=21, color="black", 
             fill=c(rep.int("blue3",2), rep.int("gray50",3), rep.int("red3",2)))
...
ggplot(data=GUNIMP, aes(x=PID, y=PE)) +
  geom_col(linewidth=1.1, width=0.8, color="black", 
           fill=c(rep.int("blue3",2), rep.int("gray50",3), rep.int("red3",2))) +
  geom_errorbar(aes(ymin=CILO, ymax=CIHI), linewidth=0.75, width=0.25) 
...
ggplot(NRA, aes(PE, RACE)) +
  ...
  geom_errorbarh(aes(xmin=CILO, xmax=CIHI), height=0, linewidth=0.75) +
...
TRUMP$TRUMP50 <- case_when(
            TRUMP$PCTTRUMP>=50 ~ "Support Trump",
            TRUMP$PCTTRUMP< 50 ~ "Oppose Trump")
...
ggplot(data=DATA, aes(x=100*PCT, y=RACE, fill=PARTY)) +
  geom_col(color="black", linewidth=1.05, width=1) +
...
theme.z <-   theme(
        ...
        panel.background   = element_rect(fill="lightsteelblue2", color="black", linewidth=0.5, linetype="solid"),
        panel.border       = element_rect(fill=NA, color="black", linetype="solid", linewidth=1.5),
        panel.grid.major.x = element_blank(), 
        panel.grid.major.y = element_blank(),
        panel.grid.minor.x = element_blank(), 
        panel.grid.minor.y = element_blank(),
...
The code below won't work if you select Run Code, but the code should work if you copy and past the code into an R session:

```{r shiny1, exercise=TRUE, exercise.lines=29}

install.packages("shiny", dependencies=TRUE, repos="http://cran.us.r-project.org")
library(shiny)

ui <- fluidPage(
        titlePanel("The effect of random error in a predictor"),
        sidebarLayout(
          sidebarPanel(
              sliderInput("integer", label="Select random error", 
                           min=0, max=100, value=0)
                     ),
        mainPanel(plotOutput("plot"), "slope of the regression line is ",
...
server <- function(input, output) {
           output$plot <- renderPlot({
             X    <- runif(1000,0,10)
             Y    <- X * 50
             Xerr <- X + rnorm(1000, 0, input$integer)
             reg1 <- lm(Y~Xerr)
             plot(Xerr, Y, xlim=c(-100,100), ylim=c(-400,600)) + 
               abline(reg1, col="red3")
           output$text <- renderText({reg1$coef[2]})
                                     }) 
                                  }
...
The code below won't work if you select Run Code, but the code should work if you copy and past the code into an R session. No need to re-copy the install.packages command or library command, if working in the same R session, but do stop the other shiny app. 

```{r shiny2, exercise=TRUE, exercise.lines=29}
...
server <- function(input, output) {
           output$plot <- renderPlot({
             X    <- runif(1000,0,10)
             Y    <- X * 25
             Yerr <- Y + rnorm(1000, 0, input$integer)
             reg1 <- lm(Yerr~X)
             plot(X, Yerr, xlim=c(-100,100), ylim=c(-400,600)) + 
               abline(reg1, col="red3")
           output$text <- renderText({reg1$coef[2]})
                                     }) 
                                  }
...
```{r trouble3, exercise=TRUE, exercise.lines=12}
